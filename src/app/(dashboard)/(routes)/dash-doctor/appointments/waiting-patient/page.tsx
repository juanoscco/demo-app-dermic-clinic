...
export default function AppointmentWaitingPatient() {
  ...
  const filteredEmployee = dataEmployee?.data?.content.filter((item: any) => item.sede.id_sede === employeeId?.sede.id_sede && item.estado);
                    ...
                    {filteredEmployee && filteredEmployee.length > 0 ? (
                      filteredEmployee
                        ...
                        .map((employee: any, j: number) => {
                          const filteredAppointment = filterAppointmentsByHourAndEmployee(hour, employee, employeeId?.sede?.id_sede, selectedDate);
